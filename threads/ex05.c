...
void *funcao_thread(void *arg) {
    int *id = (int *)arg;
    int valor_local; // Variável local para capturar o valor inicial
    
    valor_local = variavel_compartilhada; // Captura o valor no início
    printf("Thread %d - Antes da modificação: %d\n", *id, valor_local);
    variavel_compartilhada = valor_local + 10; // Usa o valor local para modificar
